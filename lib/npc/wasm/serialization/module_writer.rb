...
# module NPC
#   module WASM
#     Out = T.type_alias { T.any(StringIO, IO) }
...
#       sig { void }
#       def initialize
#         @functions = T.let([], T::Array[FuncWriter])
#         @types = T.let(FuncTypeTable.new, FuncTypeTable)
#         @imports = []
#         @exports = []
#         @start = nil
#       end
...
#       sig { params(type: T.any(Integer, IR::FuncType)).returns(Integer) }
#       def intern_type(type)
#         @types.intern(type)
#       end

#       ## Look up a function type by index.
#       sig { params(index: Integer).returns(IR::FuncType) }
#       def type_info(index)
#         @types[index]
#       end
...
#       sig { params(type: IR::FuncType).returns(FuncWriter) }
#       def func(type)
#         func = FuncWriter.new(
#           self,
#           @functions.length,
...
#       sig { params(out: String, type: IR::FuncType).returns(ModuleWriter) }
